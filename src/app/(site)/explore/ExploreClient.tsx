...
function ExploreContent({
  ...
  const [sort, setSort] =
    useState<(typeof sortOptions)[number]>("Popularity");
  ...
  const results = useMemo(() => {
    let list = books.filter((book) => {
      ...
      const matchesCategory =
        category === "All Genres" || book.category === category;
    ...
    list = [...list].sort((a, b) => {
      ...
      // Popularity is not available in the database yet.
      // Keep the existing option without inventing a rating.
      return 0;
    });

    return list;
  }, [books, query, category, author, language, sort]);
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
          ...
          <div>
            ...
            <p className="mt-4 max-w-lg text-ink-soft leading-relaxed">
              Shelf isn&apos;t just a digital library; it&apos;s a curated
              archive of human thought. From the haunting corridors of gothic
              literature to the profound depths of modern philosophy, every
              title is optimized for an undisturbed reading experience.
            </p>
